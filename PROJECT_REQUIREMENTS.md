...
| PA | **ESP-Claw AI Agent 端到端验证** | Phase 1-3 代码已完成: 15个设备 MCP tools (含 camera.capture_vision); Web Agent Chat UI (/api/agent/chat); Flutter Agent Chat 页面; LLM Vision 启用 (supports_vision=true)。待硬件验证完整链路: IM→Agent→LLM→tool_call→执行→回复 | 需硬件+LLM API key |
