...
| 2026-07-11 | +S284 **event_router 栈溢出修复 (8KB→16KB)**: Agent Chat 发消息触发 Stack protection fault (`Core 0 panic'ed in task "event_router"`, SP < bounds)。event_router 处理消息调用链深 (context providers → agent manager → LLM core)，8KB 栈不足。修复: cold-init + hot-init 两路径 `task_stack_size 8192→16384` |
