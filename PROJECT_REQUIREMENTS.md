...
| R22 | **Camera Frame UL能录制** | JPEG 帧通过 `camera_frame_chunk` uORB topic (1024 字节分块) 写入 `.ulg` 文件，支持大帧 (30-50KB JPEG) 高效录制；`tools/ulog_extract_frames.py` 和 `tools/ulog_to_video.py` 提取 JPEG 帧 + AAC 音频并合并为 MP4 | ✅ |
...
| S214 | **camera_frame_chunk 分块录制** | 替换单缓冲 `camera_frame_s` (15KB jpeg_data) 为 `camera_frame_chunk_s` (1024 字节分块，ORB_QUEUE_LENGTH=32)，消除大帧超出缓冲问题，减少 ULog 写开销，支持任意大 JPEG 帧；`tools/ulog_extract_frames.py` 和 `tools/ulog_to_video.py` 支持分块重组 | ✅ |
...
| S303 | **claw/lua_modules cJSON_Print* free() 不匹配 (45处, 17文件)** | 所有 `cJSON_PrintUnformatted()`/`cJSON_Print()` 结果用 `free()` 释放，cJSON 使用自定义分配器时堆损坏。修复: 全部替换为 `cJSON_free()`，匹配 main/ 代码惯例。`idf.py build` 验证通过 | ✅ |
| S304 | **claw volatile→atomic 迁移 (8文件, 14字段)** | 双核 ESP32-P4 上 volatile 不保证原子性和内存序。修复: `volatile bool`→`atomic_bool`，`volatile int`→`atomic_int`，`volatile bool*`→`atomic_bool*` (C11 <stdatomic.h>)，覆盖 claw_core_internal.h/cap_im_tg/qq/cap_lua_runtime/async/claw_llm_types/http_transport。`idf.py build` 验证通过 | ✅ |
| S305 | **Round 2 遗漏修复 (6文件)** | 额外 cJSON_free() 遗漏 (claw_core_events/agent_loop/claw_cap/claw_memory_storage/claw_core_llm) + lua_lvgl_private.h volatile→atomic 遗漏。`idf.py build` 验证通过 | ✅ |
...
| G5 | **app_claw Shell 移植** | 模块化 capability/lua 注册 (app_capabilities.c + app_lua_modules.c + app_claw_cli.c), 替代 main.cpp 内联初始化。上游: `common/app_claw/` | ✅ 已完成 |
| G6 | **lua_modules 移植** | Lua 脚本子系统 — esp-claw 核心特性"对话即创建"。按需移植相关模块 (audio, camera, display, storage, system, json, thread, http_server, event_publisher, call_capability)。上游: `lua_modules/` (35 个模块) | ✅ 已完成 |
| G7 | **lua_module_builder** | Lua 模块文档/测试构建工具。上游: `common/lua_module_builder/` | ✅ 已完成 |
...
| 2026-07-31 | **代码审查 Round 9 修复 (3 issues: 1 MEDIUM + 45×HIGH + 14×MEDIUM)**: **S302 (MEDIUM) uORB orb_copy() unbounded retry loop**: `while(1)` 在 generation mismatch 时可无限自旋。修复: 添加 MAX_RETRIES=3 上限 + ESP_LOGE 日志。**S303 (HIGH) claw/lua_modules cJSON_Print* free() 不匹配 (45处, 17文件)**: 所有 `cJSON_PrintUnformatted()`/`cJSON_Print()` 结果用 `free()` 释放，cJSON 使用自定义分配器时堆损坏。修复: 全部替换为 `cJSON_free()`，匹配 main/ 代码惯例。**S304 (HIGH) claw volatile→atomic 迁移 (8文件, 14字段)**: 双核 ESP32-P4 上 volatile 不保证原子性和内存序。修复: `volatile bool`→`atomic_bool`, `volatile int`→`atomic_int`, `volatile bool*`→`atomic_bool*` (C11 <stdatomic.h>)，覆盖 claw_core_internal.h/cap_im_tg/qq/cap_lua_runtime/async/claw_llm_types/http_transport。**S305 (HIGH) Round 2 遗漏修复 (6文件)**: 额外 cJSON_free() 遗漏 (claw_core_events/agent_loop/claw_cap/claw_memory_storage/claw_core_llm) + lua_lvgl_private.h volatile→atomic 遗漏。`idf.py build` 验证通过
...
| 2026-07-31 | **Camera Frame Chunk ULog 录制 + Audio ULog 录制 (S311~S313)**: **S311**: 替换 `camera_frame` 单缓冲 (15KB jpeg_data) 为 `camera_frame_chunk` 分块 (1024 字节, ORB_QUEUE_LENGTH=32)，消除大帧超出缓冲问题，匹配 esp32s31_korvo1 参考实现。**S312**: 新增 `audio_frame` uORB 主题 + `AudioUlogRecorder` 后台任务 (I2S PCM → AAC-ADTS 编码 → 发布 audio_frame)，ULog 启停时自动控制音频录制。**S313**: 新增 `tools/ulog_to_audio.py` 和 `tools/ulog_to_video.py` 提取工具，`tools/ulog_extract_frames.py` 支持 camera_frame_chunk 分块重组。`test_z_ulog_audio_camera.py` 验证音频+视频双主题录制
