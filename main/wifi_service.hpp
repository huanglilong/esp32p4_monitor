 ...
 *      NVS credentials, starts wifi_manager in STA-only mode.
 *   2. From UI/Web: scan(), connect(ssid, pass), getStatus().
 *   3. WifiService publishes uORB wifi_state automatically via
...
public:
     ...
     * Returns ESP_OK once connection sequence is initiated;
...
private:
     ...
     * both are non-empty (STA mode), false if no stored credentials. */
