...
/* ── Task config ── */
#define AUDIO_ULOG_TASK_STACK_SIZE  8192
...
void AudioUlogRecorder::_task_func(void *arg)
{
    ...
    while (self->_running.load(std::memory_order_acquire)) {
        ...
        if (read_fail_count > 0) {
            ESP_LOGI(TAG, "i2s_channel_read recovered after %u failures", (unsigned)read_fail_count);
            read_fail_count = 0;
        }
        ...
        while (remaining > 0) {
            ...
            if (enc_in_count >= enc_in_size) {
                ...
                if (esp_aac_enc_process(encoder, &in_frame, &out_frame) == ESP_AUDIO_ERR_OK
                    ...
                    /* Copy AAC data (typically ~512B, fits in 1536B buffer) */
...
void AudioUlogRecorder::stop()
{
    ...
    if (h) {
         ...
         * The task will eventually see _running=false and self-delete.
         * Do NOT free the stack — the task is still using it.
         * Detach so next start() allocates a fresh stack. */
        _task_stack = nullptr;
    } else {
        /* Task exited cleanly — safe to free the stack (TCB is reused) */
        if (_task_stack) {
            heap_caps_free(_task_stack);
            _task_stack = nullptr;
        }
