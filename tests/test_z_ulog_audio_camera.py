...
This test runs LAST (after all other tests) because it:
  ...
  3. Parses it with pyulog to verify audio_frame + camera_frame_chunk topics exist
  4. Restarts ULog to resume normal operation

It verifies:
  - ULog .ulg file contains audio_frame topic with AAC data
  - ULog .ulg file contains camera_frame_chunk topic with JPEG data (if camera connected)
...
class TestUlogAudioCamera:
    ...
    to confirm audio_frame and camera_frame_chunk topics are present with data.
    """
    ...
    def test_ulog_file_has_camera_topic(self, _ulog_path, api):
        """ULog file must contain camera_frame_chunk topic with JPEG data (if camera connected)."""
        status = api.ulog_status()
        if not status.get("camera_streaming"):
            pytest.skip("Camera not streaming (no camera connected?)")
        topics = _parse_ulog_topics(_ulog_path)
        assert "camera_frame_chunk" in topics, \
            f"camera_frame_chunk topic not found in ULog. Topics: {list(topics.keys())}"
        assert topics["camera_frame_chunk"] > 0, \
            f"camera_frame_chunk has 0 data entries. Topics: {topics}"

    def test_ulog_file_camera_has_multiple_frames(self, _ulog_path, api):
        """camera_frame_chunk should have multiple entries (5fps, 5s = ~25 frames)."""
        status = api.ulog_status()
        if not status.get("camera_streaming"):
            pytest.skip("Camera not streaming")
        topics = _parse_ulog_topics(_ulog_path)
        if "camera_frame_chunk" not in topics:
            pytest.skip("camera_frame_chunk topic not in ULog file")
        frame_count = topics["camera_frame_chunk"]
        assert frame_count >= 5, \
            f"Too few camera frame chunks: {frame_count} (expected >= 5 for 5s at 5fps)"
    ...
    def test_ulog_file_camera_frame_fields(self, _ulog_path, api):
        """camera_frame_chunk should contain expected fields: chunk_data, chunk_size, width, height, etc."""
        status = api.ulog_status()
        if not status.get("camera_streaming"):
            pytest.skip("Camera not streaming")
        ulog = ULog(_ulog_path)
        cam_data = None
        for data in ulog.data_list:
            if data.name == "camera_frame_chunk":
                cam_data = data
                break
        if cam_data is None:
            pytest.skip("camera_frame_chunk not in ULog file")
        required_fields = ["timestamp", "frame_index", "chunk_size", "width", "height"]
        for field in required_fields:
            assert field in cam_data.data, \
                f"camera_frame_chunk missing field '{field}'. Fields: {list(cam_data.data.keys())}"
        chunk_sizes = cam_data.data.get("chunk_size")
        if chunk_sizes is not None and len(chunk_sizes) > 0:
            assert chunk_sizes[0] > 0, f"chunk_size should be > 0, got {chunk_sizes[0]}"
    ...
    def test_ulog_info_tool_output(self, _ulog_path):
        ...
        if "camera_frame_chunk" in topics:
            assert "camera_frame_chunk" in output, "ulog_info output doesn't mention camera_frame_chunk"